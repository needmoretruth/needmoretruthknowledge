...
messages! {
    ...
    KeyReset { en: "start this stage over", ko: "이 단계 처음부터" },
    ...
    KeyStage { en: "next stage", ko: "다음 단계" },
    KeyStageBack { en: "previous stage", ko: "이전 단계" },
    KeyType { en: "type a number instead", ko: "숫자로 직접 치기" },
    ...
    SettingsThreadsAbout { en: "A thread is one line of work your processor runs, and one core runs one at a time. This is how many the long jobs may take; one core is left free so the screen keeps moving.", ko: "스레드는 프로세서가 돌리는 작업 한 줄이고, 코어 하나가 한 번에 하나를 돌립니다. 긴 작업이 그중 몇 개를 쓸지 정하는 값이며, 화면이 계속 움직이도록 코어 하나는 비워 둡니다." },
